...
| Update Queswa prompt — web, WhatsApp Y Dashboard, una sola fuente | `node scripts/actualizar-system-prompt-queswa.mjs` — lee `knowledge_base/system-prompt-queswa.md` y despliega `nexus_main`, `queswa_whatsapp` y **`queswa_dashboard` (tenant `dashboard`, lo lee queswa.app)** en el mismo paso. `--solo web\|whatsapp\|dashboard` · `--dry` · `--ver dashboard` (imprime la variante). Un marcador puede nombrar varios canales: `<!-- canal:web whatsapp -->`. ⚠️ Los dos scripts por canal se borraron el 4 sep 2026 — ver Reglas Críticas |
...
4. **System Prompt** - Stored in Supabase `system_prompts` table (name: `nexus_main`)
   ...
   - ⚠️ **Desde el 4 sep 2026 la fuente es `knowledge_base/system-prompt-queswa.md`, compartida con WhatsApp** (el archivo `system-prompt-nexus-main-v27_2.md` se borró; vive en git). Ver la regla del prompt único en Reglas Críticas.
