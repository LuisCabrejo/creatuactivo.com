...
|---------|--------|----------------|-----------|
| `arsenal_inicial` | creatuactivo_marketing | **v5.13** (17 jun 2026) | **v5.13** = **EAM_01 cierre humano** (rol del héroe = recibir de persona a persona a quien decidió: calidez/confianza/apretón de manos; nadie audita, ni Queswa ni el constructor). Previa **v5.12** = EAM_01 versión simple (3 pasos Expandir/Activar/Multiplicación, Activar=conversión) + **"filtrar" desterrado** + 3er Comando Maestría→Multiplicación. WHY_02 + EAM_01 sincronizados carácter por carácter con `respuestas-maestras.ts`. — WHY, STORY, VS, PERFIL, FREQ, CRED, OBJ, VOICE, EAM, CIERRE + DIASPORA. **48 fragments** (49 respuestas en .txt — FREQ_04_PUENTE no se fragmenta por el sufijo `_PUENTE`; su contenido vive en el doc padre). Historial v5.2→v5.11 → [CHANGELOG-arsenales.md](knowledge_base/CHANGELOG-arsenales.md#arsenal_inicial). |
