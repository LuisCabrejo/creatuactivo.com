...
| Update creatuactivo.com prompt | `node scripts/actualizar-system-prompt-v27.2.mjs` (despliega **v29.1 "compresion_moneda_pais"** — el archivo conserva el nombre legacy `v27.2`) |
...
|---------|--------|----------------|-----------|
| `arsenal_inicial` | creatuactivo_marketing | **v5.19** (25 jun 2026) | **v5.19** = **promesa Queswa "guía"→"madura"** (Opción B): WHY_01/WHY_02 + EAM_01 + grupo ② → *"madura en cada interesado la decisión de avanzar"* (objeto = la decisión, no la persona); calidez humana conserva "acompaña"; verbatim-lock sync con `respuestas-maestras.ts`; regla del espejo (no verbo sobre la decisión del lector en CTA). Re-fragmentado + clonado a whatsapp. Previa **v5.18** (23 jun) = EMPRESA_DIGITAL_01 Camino A + "sistema"→"puente". Previa **v5.14** = **nuevo fragmento ACTIVACION_01** ("cómo se activa mi empresa digital": proceso de arranque concreto en `<verbatim_lock>` — capitalización → paquete en oficina Gano o a domicilio → formulario sencillo físico/digital con cuenta bancaria → Centro de Mando se activa de inmediato → persona del equipo acompaña por llamada/videollamada — + filosofía "asumimos el éxito de cada Propietario como responsabilidad nuestra"). Cierra el vacío donde el modelo improvisaba con los 3 Comandos. **FREQ_03 trigger limpiado** (sin "cuánto cuesta / inversión / pasos", para no competir con ACTIVACION_01; los precios siguen en su respuesta). Handoff: "sin formularios ni trámites engorrosos" → "con un formulario sencillo y todo el acompañamiento del equipo". Previa **v5.13** = **EAM_01 cierre humano** (rol del héroe = recibir de persona a persona a quien decidió; nadie audita). **v5.12** = EAM_01 versión simple + **"filtrar" desterrado** + Maestría→Multiplicación. WHY_02 + EAM_01 sincronizados carácter por carácter con `respuestas-maestras.ts`. — WHY, STORY, VS, PERFIL, FREQ, CRED, OBJ, VOICE, EAM, CIERRE, ACTIVACION + DIASPORA. **53 fragments** (54 respuestas en .txt — FREQ_04_PUENTE no se fragmenta por el sufijo `_PUENTE`; su contenido vive en el doc padre). Historial v5.2→v5.18 → [CHANGELOG-arsenales.md](knowledge_base/CHANGELOG-arsenales.md#arsenal_inicial). |
...
4. **System Prompt** - Stored in Supabase `system_prompts` table (name: `nexus_main`)
   - **Versión activa: v29.1 "compresion_moneda_pais"** (2 jul 2026, desplegada y verificada con `leer-system-prompt.mjs`). **Compresión 33.2K→21.1K chars sin cambio doctrinal** (changelog movido a `CHANGELOG-system-prompts.md`, redundancias consolidadas) + **regla de moneda por país unificada** (Colombia → solo COP · US → USD · país sin lista/desconocido → **USD por defecto**; corrige la contradicción "ESP USD primero" que sobrevivía en BLOQUEOS DE FORMATO). Incluye lo de v28.9/v29.0 (tres fuerzas/socios, primeros principios, bisagra "Gano trabaja para usted"). ⚠️ **Promesa canónica "guía" → "madura" (25 jun 2026, Opción B):** *"Queswa explica, atiende y **madura en cada interesado la decisión de avanzar**, las 24 horas"* (objeto = la decisión, NO la persona → activo sin presionar). **Regla del espejo:** "madura la decisión" SOLO en 3ª persona (los prospectos del usuario); en CTA/interpelación al lector NO se usa verbo sobre *su* decisión (expone la persuasión) — ver [[feedback_verbos_cambio_de_estado]] · [[feedback_promesa_canonica_queswa]]. La **calidez humana** (el equipo/héroe recibe de la mano al que ya decidió) conserva "acompaña". **Previa v28.8 "calidez_sin_diagnostico"**: aforística Activar cálida + "Iniciar Diagnóstico" retirado como CTA. **Previa v28.7 "contexto_reels"** (19 jun 2026): sección "CONTEXTO DE ENTRADA — CÓMO LLEGA EL USUARIO (REELS)" — Queswa sabe que la mayoría llega tras ver un reel (home explainer + 6 nichos), con su villano narrado por perfil, y que el reel le entrega el testigo con la promesa canónica (NO "evalúa su caso / si es viable"). Acompaña el **saludo post-reel generalista** en código (`getReelGreeting()` en [src/lib/queswa-greeting.ts](src/lib/queswa-greeting.ts), gatillado por ruta `/{slug}/{nicho}` en `useNEXUSChat.ts`).
...
|---------|-------------|------------------------|
| `creatuactivo.com` | `nexus_main` | `actualizar-system-prompt-v27.2.mjs` (despliega **v29.1 "compresion_moneda_pais"** — apunta a `system-prompt-nexus-main-v27_2.md`; tanto el script como el archivo conservan el nombre legacy `v27.2`/`v27_2`. Verificar siempre con `leer-system-prompt.mjs`) |
...
- `actualizar-system-prompt-v27.2.mjs` - Deploy del system prompt actual (**v29.1 "compresion_moneda_pais"** — ~21K chars) a Supabase. El script y el archivo fuente conservan el nombre legacy `v27.2`/`v27_2`; el contenido interno es v29.1. Las versiones anteriores viven en git + `CHANGELOG-system-prompts.md`
