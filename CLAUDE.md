...
- **Fuente de verdad**: [src/lib/reels.ts](src/lib/reels.ts) — `REEL_NICHOS` (`corporativo`, `empleados`, `empresarios`, `diaspora`, `informales`), `REEL_ASSETS` (solo `{ video }`, URLs Blob), `REEL_COPY` (título/cuerpo/audiencia, versión final aprobada por Luis), `SERVILLETA_YOUTUBE_ID`, `REEL_POSTER` (`/videos/reels/poster.webp` — poster del `<video>`), `REEL_POSTER_OG` (`/videos/reels/poster.jpg` — OG image).
- **Poster único branded**: todos los reels usan **un solo poster** (no frames por-nicho, que se veían borrosos). `poster.webp` para el `<video>` en la página; `poster.jpg` (mismo arte, JPG porque WhatsApp no siempre renderiza WebP) para el OG image al compartir. Ambos en `public/videos/reels/`, **commiteados** (servidos por la app Next, no por Blob). `metadataBase` resuelve la ruta relativa del OG a absoluta.
...
- **Scripts** (en `scripts/`): ninguno genera/sube posters por-nicho (se usa el poster branded único).
  - `optimize-reels.sh` — `{nicho}.mp4` → `{nicho}-web.mp4` (CRF 23, 1080×1920, faststart). Respeta los fps de origen.
  - `upload-reels-to-blob.mjs` — sube `*-web.mp4` a Blob (`reels/{nicho}.mp4`, `allowOverwrite: true`) e imprime el mapa `REEL_ASSETS`. Requiere `BLOB_READ_WRITE_TOKEN`.
...
>
> **Estado (jun 2026):** ✅ migrado y **desplegado** en todas las superficies de cara al prospecto: **home completa** (`src/app/page.tsx` — Hero, Diagnóstico, Perfiles, Tres Caminos, Producto, Prueba de Estrés, Queswa, CTA + `CognitiveLoadComparator`), **deck `/servilleta`** (`src/app/servilleta/page.tsx` v6.2) + guion maestro v4.0 + teleprompter, **chips** (`queswa-greeting.ts`), **Camino A** (`respuestas-maestras.ts`), **WHY_01/WHY_02/EAM_01** (`arsenal_inicial.txt`, local) y los **reels de la serie de documentación** (Día 1–5). **NO revertir hacia los términos viejos.** ⏳ pendiente: FREQ_04/FREQ_04_PUENTE/PERFIL_01 + migración profunda (~200 hits en arsenales restantes + system prompt) + deploy de `arsenal_inicial` a Supabase. La tabla de abajo aún refleja el canon viejo en los términos migrados; al editar copy de cara al usuario, **siempre el léxico accesible**.
>
