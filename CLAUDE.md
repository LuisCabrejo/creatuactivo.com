...
- `wa-onboarding.ts` - **Onboarding del socio nuevo por WhatsApp**: comando `ACTIVAR`, enlace de canal, avisos de actividad. ⚠️ `constructor_id` **NO es un UUID** sino la llave de texto que comparte con el Dashboard; y el enlace es **`/{slug}/queswa`**, nunca `/{slug}` —esa página no existe y da 404— porque además valida el slug antes de redirigir. Ver el handoff del 17 ago
