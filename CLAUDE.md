...
| Recomponer las fotos de producto | `node scripts/componer-imagenes-producto.mjs` (22 imágenes 1080×1080) |
...
- ❌ **NO narrar en la nota de versión del arsenal lo que se acaba de retirar.** La cabecera `**Versión actual:**` vive **dentro del documento padre que se indexa en Supabase**, así que citar allí la cifra falsa, la frase vetada o la historia eliminada la devuelve al corpus. Pasó tres veces el 8 ago. **Se enuncia el criterio, no el ejemplo**; el detalle va al [CHANGELOG](knowledge_base/CHANGELOG-arsenales.md), que no se indexa
...
- ⚠️ **`DESTINO_MAP` (cuesta horas):** un destino que no esté en el mapa —ni en `REEL_NICHOS` ni `manifiesto`— **cae al fallback `redirect(/{slug})`, que es la mini-landing, sin 404**. Síntoma: *"el enlace `/{slug}/X` lleva a la mini-landing"*. Al añadir un enlace amigable en el Dashboard (`src/lib/arsenal.ts`), agregue SIEMPRE su destino en [src/app/[slug]/[destino]/page.tsx](src/app/[slug]/[destino]/page.tsx).
...
**Menú** ([StrategicNavigation.tsx](src/components/StrategicNavigation.tsx), array `directLinks`): Nosotros (`/manifiesto`) · Tecnología · Presentación (`/servilleta`) · Insights (`/blog`) + CTA **"Suscríbete"**. ⚠️ **Los rótulos no coinciden con sus rutas a propósito** (jun 2026): el menú nombra *qué encuentra el visitante*, no la ruta técnica. `/presentacion-empresarial` es herramienta interna 1-a-1 y **no** está en el menú — no confundirla con el item "Presentación".

**Indexadas:** `/`, `/fundadores`, `/blog/*`, `/tecnologia`, `/sistema/productos`, `/paquetes`. **noindex:** `/manifiesto`, `/prueba`, `/12-niveles`, `/lexico`, decks internos.
...
- **Formación** (8 ago 2026): sección **Maestría** en queswa.app — liderazgo, comunicación, maestría para el éxito y maestría para administrar los recursos; más espacios en vivo con profesionales de la salud y con socios que ya recorrieron el camino. *Maestría* aquí nombra la sección del producto, no el Comando renombrado
...
- `wa-channel.ts` - **Capa única de canal WhatsApp** (Meta Cloud API): `sendText` (acepta `responderA` para citar) · `sendImage` · `sendTemplate` · `sendReplyButtons` · `sendFlow` · `marcarLeidoYEscribiendo` (visto azul + "escribiendo…" en UNA llamada; se cae sola a los 25 s, por eso el webhook la renueva cada 20 mientras el motor trabaja) · `listTemplates` · `getPhoneAsset`. Único lugar con `WHATSAPP_SYSTEM_TOKEN`. (`whatsapp-meta.ts` + `sendpulse.ts` borrados jul 2026)
...
⚠️ **El activo del usuario se nombra "canal de distribución"** (doctrina jul 2026, ratificada 14 ago — fuente: cabecera de `arsenal_inicial.txt` y prompt `queswa_whatsapp`). *"Empresa digital"* fue el término jun–jul 2026 y **NO se usa en primer contacto**:

> ⚠️ **"Empresa digital" NO está prohibida — está acotada, y la diferencia importa** (auditoría 17 ago 2026). Un conteo bruto sobre `arsenal_inicial.txt` da ~35 ocurrencias y **asusta sin razón**: al abrirlas, casi todas están en notas de versión, en cabeceras `[Concepto Nuclear]` y en **líneas de disparo** —donde es correcta, porque son las palabras que el prospecto escribe—. En cuerpo servido quedan pocas, y el Director las revisó una por una el 17 ago: **se conservan**. Es correcta en `EMPRESA_DIGITAL_01` (alguien preguntó por el término), en `ACTIVACION_01`, en las tres de DIÁSPORA, y en FREQ_10 · FREQ_14 · FREQ_18, que responden a quien **ya está metido en la conversación** y por tanto ya se ganó la categoría. **No abrir un barrido de este término sin contar antes dónde está**: el arsenal está sano.
 la categoría se gana después de que la persona vio el mecanismo ("las categorías se ganan, no se anuncian" — el prospecto rellena el contenedor vacío con pirámides o cursos). La Home v14, `/paquetes` v4.1 y `/planes` ya están migradas; el sustituto de *red* también es **canal de distribución**.
...
- **cambiar horas/tiempo por dinero** (villano) → el villano se **NARRA**: *"al día siguiente de que le entra la plata, ese dinero ya tiene dueño — el banco, las cuotas, los recibos"* + remate obligatorio *"le pasa igual al que gana dos millones y al que gana veinte"*. ⚠️ *Dependencia* es el nombre **analítico** nuestro, no texto para el prompt: escrito como etiqueta es lo único que el modelo puede copiar, y así se amputó el villano del canal (19 ago 2026). La **promesa** sí usa "no depende de su presencia" ([[feedback_horas_no_son_el_villano]])
