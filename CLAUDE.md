...
- ⚠️ **El proyecto que sirve creatuactivo.com se llama `creatuactivo-com`** (repo `LuisCabrejo/creatuactivo.com`, despliega por GitHub). Existe además uno llamado **`marketing`** —abandonado desde dic 2025, sin repo, en `marketing-xi-dun.vercel.app`— y el `.vercel/project.json` de esta carpeta apunta a ESE. Como `.vercel` está en `.gitignore` el enlace es por máquina, así que no afecta los despliegues (entran por GitHub); pero **un `vercel deploy` desde aquí publica en el proyecto equivocado**
- ⚠️ **Un `engines` en `package.json` pisa en silencio la versión de Node del panel de Vercel.** El aviso de fin de vida de Node 20 (ago 2026) salió de ahí: el Dashboard traía `"node": "20.x"` mientras su panel decía 22.x, así que el build corría en 20 y el panel mentía. **Al diagnosticar versión de Node se lee el `package.json` ANTES que el panel** — el log del build lo confirma con un `Warning: Due to "engines"…`. Hoy los 12 proyectos de la cuenta están en **24.x** y este repo no tiene pin (obedece al panel)
...
⚠️ **El activo del usuario se nombra "canal de distribución"** (doctrina jul 2026, ratificada 14 ago — fuente: cabecera de `arsenal_inicial.txt` y prompt `queswa_whatsapp`). *"Empresa digital"* fue el término jun–jul 2026 y **NO se usa en primer contacto**: la categoría se gana después de que la persona vio el mecanismo ("las categorías se ganan, no se anuncian" — el prospecto rellena el contenedor vacío con pirámides o cursos). La Home v14, `/paquetes` v4.1 y `/planes` ya están migradas; el sustituto de *red* también es **canal de distribución**.
...
- **cadencia de pago mezclada** → el consumo es **mensual**, Gano liquida **cada viernes**. Decir *"le entra mes a mes"* obliga a desenredarlo después ([[reference_gano_paga_semanal]])
