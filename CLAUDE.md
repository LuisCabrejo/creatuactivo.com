...
| Update Queswa prompt — web Y WhatsApp, una sola fuente | `node scripts/actualizar-system-prompt-queswa.mjs` — lee `knowledge_base/system-prompt-queswa.md` y despliega `nexus_main` y `queswa_whatsapp` en el mismo paso. `--solo web\|whatsapp` · `--dry` · `--ver web` (imprime la variante). ⚠️ Los dos scripts por canal se borraron el 4 sep 2026 — ver Reglas Críticas |
...
**Scripts:** `actualizar-system-prompt-queswa.mjs` (despliega el prompt a los dos canales) · `clonar-arsenal-whatsapp.mjs` (ver la advertencia de arriba).
...
|---------|-------------|------------------------|
| `creatuactivo.com` + WABA | `nexus_main` · `queswa_whatsapp` | `actualizar-system-prompt-queswa.mjs` — un solo archivo fuente (`system-prompt-queswa.md`), dos filas. Verificar siempre con `leer-system-prompt.mjs` |
