...
- `wa-pareja.ts` - **La consulta con la pareja** (24 ago 2026): detector de «lo consulto con mi esposa», el **enlace para la pareja** (wa.me a Queswa con texto pre-llenado: slug del socio + «soy la pareja de {nombre}» — la misma pieza del enlace del socio, sin infraestructura nueva), el turno dictado con **cierre por opciones** («¿mañana, o en dos días?» — abierto, el 100 % responde «yo le aviso»), la apertura para cuando la pareja llega (la estándar con sus botones y una línea de reconocimiento; nunca una pregunta abierta a quien apenas ve la información) y el aviso al socio con el plazo. ⚠️ Cableado en el webhook pendiente — el nodo de ambivalencia del otro agente detecta el mismo caso y debe delegar aquí
