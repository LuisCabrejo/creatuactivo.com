...
- [x] **El saludo de socia SALE** — confirmado con tráfico real el 11 sep 11:13: Patricia recibió por primera vez su enlace, tras tres visitas sin él. ⚠️ **Pero el saludo hace `return` y se llevó su petición por delante** (pidió que le redactaran una presentación). Es el mismo defecto que en el primer contacto del prospecto: quien llega preguntando no debería recibir solo una presentación. Arreglo propuesto: si el mensaje trae petición, saludar **y** seguir al motor.
- [ ] **El candado dictado ignora el modo socio — CONFIRMADO en producción con una persona real** (11 sep 11:19). Patricia reintentó y recibió `WHY_01` **literal**: la respuesta de *«¿qué es CreaTuActivo?»* escrita para un prospecto, a una socia que pidió que le **redactaran** algo. El bloque que emite el candado (`route.ts`, buscar `_esCandadoSolitario`) excluye `whatsapp_salud_*` y **nada más**, así que el `pageContext: whatsapp_socio` no lo frena y el modelo nunca ve el turno. **Arreglo de una línea:** excluir también el modo socio, como ya se excluye la salud. Es el fallo de mayor alcance del canal hoy — le pasa a cualquier socio cuya consulta caiga en un fragmento con candado.
- [ ] **«presentación» solo cuenta como pieza si lleva calificador.** `RE_PIDE_PIEZA` exige *presentación (para|en canva|de ventas|comercial|publicitaria)*, así que *«una presentación sugestiva»* —como lo escribió Patricia— y *«una presentación del negocio»* pasan de largo. Medido el 11 sep.
