...
## 8. Cuarta vuelta — 10-11 sep 2026: la experiencia del DISTRIBUIDOR

> 🩺 **Un build puede morir por tiempo aunque el código esté bien** (11 sep 2026): `BUILD_EXCEEDED_MAXIMUM_TIME` con el compilado terminado en 31 segundos y el cuelgue en *Deploying outputs*. Y cuando eso pasa, **los commits que llegan detrás se CANCELAN** en cadena, así que un push nuevo no lo arregla: hay que mirar el estado. Con `VERCEL_TOKEN` en `.env.local`: `curl -s -H "Authorization: Bearer $VERCEL_TOKEN" "https://api.vercel.com/v6/deployments?app=creatuactivo-com&limit=8"` da estado y sha de cada uno.
>
> 🩺 **Antes de creer que un arreglo está vivo, pregúntele al motor qué commit sirve:** `curl -s https://creatuactivo.com/api/nexus` trae `commit` y `desplegado_en` desde el 11 sep 2026. `API_VERSION` es una constante escrita a mano y lleva meses sin cambiar: no dice nada. Sin sesión del CLI de Vercel no había otra forma de saberlo, y se perdió media hora comprobando un cambio contra una versión anterior.

Auditoría a pedido del Director: *«me comentó que ha estado creando mensajes de contacto»*. **Patricia Reyes es socia desde julio y en tres visitas en ocho días la atendieron como prospecta las tres veces.** El 2 sep llegó por el enlace de ganocafe-online preguntando por productos y recibió la apertura de negocio; el 8 sep abrió con cáncer de colon y quimioterapia —preparándose para lo que le van a preguntar a ella— y recibió dos veces la derivación escrita para quien cuenta su condición, después el plan como a prospecta y la cédula para vincularla; el 9 sep le redactaron cinco mensajes de contacto sin darle nunca su enlace. Su ficha quedó como prospecta caliente, interés 84, en el pipeline de quien la invitó.

**El mecanismo, no el copy.** El canal exigía slug para reconocer a un socio y **8 de 18 socios activos no lo tenían**; y aun reconociéndola, la puerta del saludo —donde vive el enlace— exigía que **no tuviera ficha de prospecto**, y ella tenía una desde el 2 sep. El guardarraíl de salud, que corta el turno, corría **antes** de identificar al socio, así que un distribuidor no llegaba nunca al modo socio.

**Lo que quedó desplegado** (`1d67210`, `137f7ef`, `452a936`): el slug se asigna por defecto (`asignarSlugPorDefecto`, backfill de 7 socios) · el prospecto que pasa a distribuidor queda configurado como distribuidor (`convertirProspectoEnSocio`: `user_id`, `stage=maestria`, `es_socio`, sin temperatura; backfill de 4 fichas) · el saludo con su enlace sale aunque haya ficha previa, y **una sola vez** (`saludo_socio_en`) · el hilo se corta en `socio_desde` · el motor recibe `socioEnlace` en modo socio · el nodo 2.49 cubre tarjeta de presentación, folleto, propuesta comercial, la invitación dirigida a un público y **la repregunta sin verbo tras la negativa** · la detección del socio subió a 1.38 y `RECHAZO_SALUD_SOCIO` le da la línea en vez de la derivación. Arnés: `npx tsx scripts/prueba-experiencia-socio.mts`.

### 8.1 Checklist de pendientes del distribuidor

- [ ] **El socio que prospecta COMERCIOS no tiene camino, y es lo que Patricia está haciendo de verdad.** No aplica Compartir y Recibir: vende a restaurantes, tiendas naturistas y cafés **como proveedora**, y usa a Queswa de redactora de propuestas comerciales — incluida la respuesta a un lugar de onces que le dijo que no. El esqueleto de `wa-redaccion-socio.ts` está escrito para **un mensaje a un conocido** (saludo ritual, el porqué él, permiso, sin enlace), que es justo lo contrario de una propuesta a un negocio que no lo conoce. Hoy esas peticiones caen al motor y se componen libres, sin candado, sin arsenal y sin que los dos guardarraíles vean nada raro. **Decisión pendiente del Director: qué le ofrecemos a un socio que prospecta comercios** — si es un esqueleto propio, si es material aprobado del Dashboard, o si el canal dice que no y lo deriva. Mientras tanto el nodo 2.49 frena las piezas publicitarias, pero **no** la propuesta de proveeduría.
- [x] **La conversión se deshacía sola — corregido el 11 sep.** El scoring le reponía `momento_optimo` e `interest_level` **en cada mensaje**, en dos bloques distintos de `route.ts` (el PASO 7 y un bloque legacy posterior que lo pisaba con la escala vieja 0–10). Patricia volvió a quedar «tibia» dos mensajes después de convertirla. Hoy ninguno de los dos toca la ficha si `device_info.es_socio`, y `convertirProspectoEnSocio` es idempotente: repara la temperatura repuesta en vez de saltarse la ficha. ⚠️ **Defecto anterior anotado y NO tocado:** ese bloque legacy juzga con umbrales de 0–10 un valor que hoy es 0–100, así que un prospecto con 10 sobre 100 sale «caliente». El Dashboard lee esos campos; corregirlo mueve su pipeline.
- [x] **El saludo de socia SALE** — confirmado con tráfico real el 11 sep 11:13: Patricia recibió por primera vez su enlace, tras tres visitas sin él. ⚠️ **Pero el saludo hace `return` y se llevó su petición por delante** (pidió que le redactaran una presentación). Es el mismo defecto que en el primer contacto del prospecto: quien llega preguntando no debería recibir solo una presentación. **Corregido el 11 sep:** si el mensaje trae petición, el saludo va aparte —persistido con el turno de usuario vacío, para no duplicarlo en el historial— y el turno sigue al motor.
- [x] **El candado dictado ignoraba el modo socio — CORREGIDO y VERIFICADO en producción el 11 sep** (`b45324e`). Patricia reintentó y recibió `WHY_01` **literal**: la respuesta de *«¿qué es CreaTuActivo?»* escrita para un prospecto, a una socia que pidió que le **redactaran** algo. El bloque que emite el candado (`route.ts`, buscar `_esCandadoSolitario`) excluye `whatsapp_salud_*` y **nada más**, así que el `pageContext: whatsapp_socio` no lo frena y el modelo nunca ve el turno. **El arreglo:** `_esCandadoSolitario` excluye `whatsapp_socio` igual que ya excluía la salud, así que el turno llega al modelo con las instrucciones del modo socio delante. Es el fallo de mayor alcance del canal: le pasa a cualquier socio cuya consulta caiga en un fragmento con candado. **Verificado con el build vivo:** la misma petición devuelve `fragment_vector_search` y el modelo responde a lo que se le pidió, reconociendo a la socia. ⚠️ **Y la verificación costó media hora por una razón que no era el código:** el build de ese commit murió con `BUILD_EXCEEDED_MAXIMUM_TIME` —compiló en 31 s y se colgó en *Deploying outputs*—, los tres siguientes se cancelaron por encadenarse, y hasta el cuarto no hubo producción nueva. **Se comprueba con `curl -s https://creatuactivo.com/api/nexus` (campo `commit`, añadido ese día) antes de medir nada.**
- [x] **«presentación» solo contaba como pieza si llevaba calificador — corregido.** `RE_PIDE_PIEZA` exige *presentación (para|en canva|de ventas|comercial|publicitaria)*, así que *«una presentación sugestiva»* —como lo escribió Patricia— y *«una presentación del negocio»* pasan de largo. Hoy `presentaci[oó]n` cuenta sola, y si el mensaje **nombra un producto** vuelve a ser el envase (lo responde el catálogo) salvo que además pida una pieza. ⚠️ **Y los verbos no toleraban la tilde mal puesta:** ella escribió *«Redácta»* y el patrón solo traía `redacta`, así que su petición se fue al vector. Es la regla de no exigir ortografía perfecta, incumplida en un patrón nuevo.
- [ ] **«Revísame este mensaje» no tiene nodo, y el candado dictado amplifica el error** (11 sep 2026, medido contra producción). Es lo más probable que haga un socio que YA tiene su texto escrito. Hoy la petición cae al vector con el texto pegado como consulta: el de Patricia enrutó a `CLIENTE_VIP_01` —que **lleva candado**— y, como desde el 9 sep el candado lo **emite el backend sin modelo** (`candado_dictado`), el socio recibió la ficha completa de Cliente Preferencial con precios, en modo socio y sin que el modelo pudiera corregir el rumbo. `detectarPidePieza` no reconoce *revísame · corrígeme · qué tal quedó*, y el esqueleto de `wa-redaccion-socio.ts` está escrito para **redactar**, no para **revisar**. Dos capas que arreglar: un detector de revisión que lleve al esqueleto, y decidir si el candado dictado debe ceder cuando el `pageContext` es `whatsapp_socio`.
- [ ] **El texto viejo sigue circulando.** Patricia está mandando a contactos reales el mensaje que Queswa compuso libre el 9 sep (antes del arreglo). Pasa **los cuatro filtros de salida** y falla **trece** comprobaciones de doctrina, entre ellas el marco del consumo diario (vetado en agosto) y *«cada taza que toman le paga a alguien»*, que es la escalera de gente dicha en el primer mensaje. El sistema ya no lo produce, pero lo producido no se retira solo: hay que reemplazarlo con ella.
- [ ] **«¿Cuántos países tiene Gano Excel?» → Queswa dice que NO TIENE EL DATO y deriva al socio** (batería de 42, 11 sep). Es una constante canónica —más de 60— y vive en cinco fragmentos (`FREQ_07`, `FREQ_10`, `CRED_02`, `CRED_04`), pero la pregunta enruta a `DIASPORA_02` (0.569) y `CRED_05` (0.540), que hablan de **los 16 países de CreaTuActivo** y del nombre de la marca. Con ese contexto el modelo responde que no lo sabe. **Es enrutamiento, no copy**, y además incumple la regla de no derivar a un humano por un dato que tenemos. Medir antes de tocar disparadores.
- [ ] **«¿Los productos tienen registro sanitario?» devuelve la ficha de qué es el Ganoderma** (misma batería). El INVIMA está en el catálogo; la pregunta no llega a él.
- [ ] **Reconocer a quien vuelve** (heredado de §6): Patricia había escrito el 2 sep y Queswa no dio señal de recordarla. No existe en ninguna familia.
