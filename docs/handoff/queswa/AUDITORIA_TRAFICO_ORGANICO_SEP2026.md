...
## 8. Cuarta vuelta — 10 sep 2026: la experiencia del DISTRIBUIDOR
...
- [x] **El candado dictado ignoraba el modo socio — CONFIRMADO en producción y CORREGIDO el 11 sep** (`b45324e`). Patricia reintentó y recibió `WHY_01` **literal**: la respuesta de *«¿qué es CreaTuActivo?»* escrita para un prospecto, a una socia que pidió que le **redactaran** algo. El bloque que emite el candado (`route.ts`, buscar `_esCandadoSolitario`) excluye `whatsapp_salud_*` y **nada más**, así que el `pageContext: whatsapp_socio` no lo frena y el modelo nunca ve el turno. **Corregido:** `_esCandadoSolitario` excluye ahora `whatsapp_socio` igual que la salud, así que el turno llega al modelo con las instrucciones del modo socio delante. Era el fallo de mayor alcance del canal: le pasaba a cualquier socio cuya consulta cayera en un fragmento con candado.
