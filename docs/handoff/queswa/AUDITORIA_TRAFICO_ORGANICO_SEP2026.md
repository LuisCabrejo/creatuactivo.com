...
- [ ] **El candado dictado ignoraba el modo socio — CONFIRMADO en producción, arreglo escrito y PENDIENTE DE VERIFICAR** (`b45324e`). Patricia reintentó y recibió `WHY_01` **literal**: la respuesta de *«¿qué es CreaTuActivo?»* escrita para un prospecto, a una socia que pidió que le **redactaran** algo. El bloque que emite el candado (`route.ts`, buscar `_esCandadoSolitario`) excluye `whatsapp_salud_*` y **nada más**, así que el `pageContext: whatsapp_socio` no lo frena y el modelo nunca ve el turno. **El arreglo:** `_esCandadoSolitario` excluye `whatsapp_socio` igual que ya excluía la salud, así que el turno llega al modelo con las instrucciones del modo socio delante. Es el fallo de mayor alcance del canal: le pasa a cualquier socio cuya consulta caiga en un fragmento con candado. ⚠️ **No dar por bueno sin medirlo:** seis sondas seguidas tras el despliegue seguían devolviendo `candado_dictado`, y el código desplegado es el correcto. O el despliegue tardó más de lo que parecía, o hay una vía que el razonamiento no ve. **Se comprueba con `curl -s https://creatuactivo.com/api/nexus` (campo `commit`) y después una sonda con `pageContext: whatsapp_socio`.**
