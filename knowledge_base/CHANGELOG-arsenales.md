...
**Rompían una regla.** `INV_05`: *«El activo es su sistema, en acción»* —el activo como otro nombre del sistema— → *«Lo que se compra una vez, y lo que se repite»*; y *«su "costo operativo"»* → *«lo que mantiene su código activo, y la recibe en producto: no es una cuota»*. `INV_03`: *«Posición heredable (es un ACTIVO)»* → *«Su código, a su nombre»* (sin causa, y la herencia fuera de `FREQ_05`); *«donde opera el sistema»* → *«donde está»*; *«Sistema de duplicación automatizado»* → *«La estrategia de Los 12 Niveles»*. `NIVELES_07`: fuera *«los expertos recomiendan»* y el cierre *«¿cuántos nombres tiene en su lista para invitar?»* (la lista de nombres, marcador del gremio, y pregunta sin fragmento que la atienda) → *«¿Le muestro con cuánto se empieza?»*. `NIVELES_06` **reescrita entera**: encadenaba cuatro pasos a un ingreso, compadecía (*«no cuando "tenga más dinero"»*) y cerraba con presión (*¿Arrancamos hoy?*); ahora el Kit existe para que empezar no dependa del presupuesto, y cierra al simulador con la tarifa del Kit.

**Léxico.** *Activo* dicho de la persona pasa al código en `INV_01` (*abrir su código*), `INV_04` (*mantiene su código activo*) e `INV_06`. *«El Kit entra al plan»* → *«da acceso al plan»* (`NIVELES_03`, `INV_00`). `INV_02`: *«la tecnología es GRATIS para todos»* → *«creatuactivo.com y queswa.app vienen incluidos con cualquiera de los cuatro»*. `INV_04` pierde la negación *«no es conectar sin parar»*. Las dos negaciones del candado de `NIVELES_02` (*no los regala nadie* · *no paga por gente vinculada*) **se conservan a criterio del Director**: ahí el lector ya trae la sospecha.

**Registro.** Mayúsculas de urgencia a minúscula en `NIVELES_03`, `05`, `07` e `INV_02`; `NIVELES_05` cambia el bloque *«lo que sí es crítico es que arranque HOY»* por una frase. `NIVELES_02` liquidaba dos veces; sobra la línea aparte. La cabecera pasa de veinte versiones a la actual y las dos previas (el historial se movió aquí abajo, verbatim), el título deja de decir *BAUTIZO EMPRESARIAL*, y el conteo dice 16 respuestas (INV son siete, no seis). Índices sin cambio salvo ninguno; despliegue con el orden sin ventana.

#### Historial movido desde la cabecera (verbatim, 13 sep 2026)

**v6.16 (3 sep 2026):** **El apalancamiento reemplaza el ejemplo de clientes propios, y el GEN5 pasa a ser un SEGUNDO botín tras la estrategia** (Director). El ejemplo de recurrentes al 10% ($2.520.000 por 200 clientes) anclaba modesto y reforzaba «una red de miles es trabajo personal» —la objeción que la estrategia debe DESARMAR—. En su lugar, el **apalancamiento**: sin cifra, explica que la ganancia crece porque cada distribuidor construye su propia red de consumo y el ingreso es un % de todo ese volumen; cierra ofreciendo la estrategia. Y el **reorden del cierre**: la tabla NIVELES_02 (y el simulador de renta) ya no cierra ofreciendo la vinculación sino **«¿Le muestro la segunda forma de ganar en este negocio?»** → el «sí» dicta el GEN5 como bono directo por paquete (webhook 2.355), su «sí» dicta el ejemplo de cifras GEN5, y ese cierra con «¿cómo se vincula?». El GEN5 queda **después** de la estrategia, nunca antes, y solo lo ve quien recorre la avenida completa. El texto del apalancamiento es el que MÁS se duplica —el prospecto lo repite al explicar el negocio—. Fundamento → `FLUIDEZ_DEL_FLUJO_12NIVELES_SEP2026.md`.

**v6.15 (3 sep 2026):** **El ejemplo de recurrentes pasa al 10% — una sola tarifa en toda la avenida.** Corría al 17% del Visionario ($428.000 / $4.3M) y luego la estrategia al 10% del Kit: dos tarifas, un salto, un número que se encogía a mitad de camino, y un puente-parche para explicarlo. Ahora el ejemplo corre al **10%** ($252.000 / $2.520.000), que es la tasa base real de Gano Excel —donde arranca todo distribuidor— y la misma de la estrategia; se retiró la coletilla «la tarifa con la que arranca todo distribuidor» (sobra: el desinterés en venderle el paquete grande es lo que da la confianza). Fundamento conductual (reactancia, anclaje, processing fluency, vacío de curiosidad) → `docs/investigaciones/resultados/FLUIDEZ_DEL_FLUJO_12NIVELES_SEP2026.md`. Los paquetes empresariales quedan solo por pull; el upgrade lo maneja el Director en el 1-a-1.

**v6.14 (3 sep 2026):** **La estrategia pasa a la avenida principal — el ejemplo de INGRESOS RECURRENTES también cierra ofreciéndola** (Director). El camino usual es cómo funciona → recurrentes, y ese nodo lo toca todo el mundo; con la estrategia solo tras el GEN5, quien iba por defecto nunca la veía (recorrido de Edilberto: 8 turnos sin ella). Ahora **los dos ejemplos —renta y GEN5— y el simulador de renta cierran con «¿Le muestro la estrategia de los 12 Niveles…?»**; el «sí» dicta NIVELES_01 con su simulador de niveles. Se retiró el puente renta→GEN5 (era el último empujón a GEN5 sin que nadie lo pidiera); el GEN5 sigue respondiéndose por pull.

**v6.13 (2 sep 2026):** La estrategia entra al camino usual por el GEN5 (superado por v6.14, que la lleva también a los ingresos recurrentes). Se descartó ponerla en los botones de la apertura: silueta ante Meta y WhatsApp solo permite tres botones.

**v6.12 (1 sep 2026):** El Director presenta la estrategia en su 1-a-1 como *el plan estratégico que se lanza el 1 de septiembre* y manda a sus prospectos a preguntarle a Queswa por ella. Entran los nombres con que van a llegar —plan de septiembre, plan estratégico, plan nuevo, el que están lanzando— al índice de NIVELES_01 (y al clasificador, al dictado del webhook y al prompt). Nace `NIVELES_09` para la pregunta que ese encuadre provoca: ¿hasta cuándo va? **No tiene fecha de cierre: nace hoy para quedarse.** Queswa no habla de lanzamiento — eso lo dice el Director en su chat. NIVELES (9) + INV (6) = 15 respuestas.

**v6.11 (1 sep 2026):** El índice de `NIVELES_04` carga la oferta literal con que cierran la tabla y el simulador («¿Le muestro cómo se vincula a la estrategia?»). Medido: puesto 1 por vector (0.517). ⚠️ En el canal ese «sí» además se dicta desde el webhook (bloque 2.36) porque un patrón del clasificador se lo llevaba a arsenal_inicial antes de llegar al vector.

**v6.10 (1 sep 2026):** El índice de `NIVELES_01` carga las formas en que se oye mal el nombre —plan de dos ciclos, de dos niveles, de 12 días / semanas / meses—. El clasificador ya las enrutaba aquí, pero el modelo, al no reconocer el nombre, componía un «no es un término que manejemos» en vez de entregar el candado. El prompt del canal (v4.32) las nombra también.

**v6.9 (1 sep 2026) — EL HILO SE CIERRA SOBRE SÍ MISMO (doctrina del Director).** Este plan existe para desarmar tres objeciones mentales: *«esto es para ganar en 50 años»*, *«toca hacerlo con el paquete de $1.000»* y *«una red de miles es trabajo personal»*. El ejemplo demuestra lo contrario: una organización de más de 8.000 puede empezar con dos distribuidores, las ganancias vienen de crecer en escala, y se puede iniciar desde el paquete más pequeño. Observación de campo: **mostrar el paquete grande produce procrastinación, y de 50 que procrastinan vuelve uno**; si 50 ven que es estratégico iniciar ya en el Kit, ocurre lo importante — se construye la base de consumo y distribución, que es el objetivo (las ganancias recurrentes, no los bonos de inicio). Quien inicia con el Kit y desarrolla el proyecto luego se pasa a un paquete empresarial por su cuenta. **Por eso en este hilo NO se muestran las otras opciones salvo que la persona las pida:** sale la tabla de tarifas de NIVELES_02 y la frase de los empresariales de NIVELES_03; los cierres encadenan dentro de la estrategia (tabla → después → vinculación). Misma regla en el prompt del canal.

**v6.8 (1 sep 2026):** **La proyección pasa a MENSUAL, y la cuenta queda coherente con el consumo** (corrección del Director). La tabla ponía $51.609.600 «a la semana» en el nivel 12: era la semana en que se compran los 4.096 kits de ese nivel —un evento de construcción—, no el ingreso del sistema consumiendo, que con 8.190 distribuidores a 4 cajas al mes es 229.320 CV por lado → **$103.194.000 al mes** (lo que NIVELES_08 ya decía). Y la vieja columna «acumulado» ($25.200 × (2ⁿ−1)) era exactamente el mensual del sistema con ese tamaño (T × $12.600), mal rotulada durante meses. NIVELES_01 y NIVELES_02 lo dicen ahora como regalía mensual; el Flow y `respuestaNiveles()` calculan igual. Cifras del plan (CV por caja, tasa, 10%) intactas.

**v6.7 (1 sep 2026):** `NIVELES_01` y `NIVELES_02` pasan bajo **candado** `<verbatim_lock>` (cierre afuera, como manda el contrato). Ejercicio de punta a punta contra producción: sin candado, el modelo reescribía NIVELES_01 degradando el sistema a «café y suplementos», y ante el «sí» a la tabla —cuyo cierre compuesto se fue a COMP_BIN_08— inventó una tabla entera con cifras que no existen. El candado se entrega tal cual y solo. El índice de NIVELES_02 carga además el cierre literal de NIVELES_01 (medido: 0.533, puesto 1). Cifras intactas.

**v6.6 (1 sep 2026):** `NIVELES_01` se simplifica a los conceptos nucleares (texto del Director): tres párrafos cortos, cierre hacia la tabla, y el $103M vuelve a la primera respuesta — pegado a su origen en la misma frase. Y **«mínimo dos»** entra como forma canónica de la duplicación (aquí y en NIVELES_02): *conecta dos* es la geometría rígida de la pirámide; *mínimo dos* es un piso, como habla un negocio, y vuelve la proyección un escenario base.

**v6.5 (31 ago 2026):** Nace `NIVELES_08` — el «después»: cuánto queda ganando cuando el sistema ya está construido. No tenía puerta, y la cuenta se verificó antes de escribirla: con los 8.190 consumiendo sus cuatro cajas, el mensual del sistema completo es el mismo $103M del recorrido — la misma aritmética (4.095 unidades de 56 CV al 10%). NIVELES (8) + INV (6) = 14 respuestas.

**v6.4 (31 ago 2026):** La tabla de `NIVELES_02` pivota al consumo (Director): las filas cuentan **distribuidores consumiendo → regalía semanal**, y salen las columnas de nivel, entrantes por nivel y acumulado — la escalera con columnas. La duplicación 2×2 baja a prosa como lo que construye el tamaño, y el $103M queda junto a su origen. Misma decisión aplicada en la pantalla REGALIA_EQUIPO del Flow y en `respuestaRegalia()`. Cifras intactas.

**v6.3 (31 ago 2026):** El índice de `NIVELES_02` gana la pregunta por las ganancias mensuales dentro del hilo («cuáles serían las ganancias mensuales en esta estrategia») — en la prueba del Director esa consulta caía en compensación y el modelo pivotaba al GEN5, que el Kit ni incluye. Medido: con el hilo anclado gana 0.525 contra 0.402; la consulta sin hilo se queda en compensación (su dueño correcto) y COMP_BIN_08 conserva las suyas. El marco de la respuesta —las ganancias las determina el movimiento de producto, no el calendario— vive en el prompt v4.27.

**v6.2 (31 ago 2026):** `NIVELES_05` y `NIVELES_07` dejan de definir «su rol» con fórmula propia (Director): el rol de cara al prospecto son siempre las dos acciones —Compartir y Recibir— y las dicta EAM_01. En este arsenal, *conectar* queda solo como descripción del mecanismo 2×2, nunca como definición del rol.

**v6.1 (31 ago 2026):** `NIVELES_04` deja de mandar afuera a quien quiere vincularse: la vinculación se radica en el canal con los cuatro datos, la misma condición de quien inicia con paquete empresarial (Director). Sus «dos vías» eran la misma vía escrita dos veces. El bloque de datos coincide con `pedirDatos()` de wa-radicacion.ts a propósito — red de respaldo del detector de volición.

**v5.6 (31 ago 2026):** `NIVELES_02` carga en su índice las ofertas de cierre literales — la escrita en NIVELES_01 y la de la tabla nivel a nivel. En WhatsApp una aceptación pelada busca con la última pregunta del bot, y sin la oferta en el índice el «sí» a la tabla de los niveles se iba al ejemplo mensual del Binario (prueba del Director, 31 ago). Medido con arnés antes de desplegar: las dos ofertas de cadena pasan al puesto 1 y COMP_BIN_08 mantiene las suyas.

**v5.5 (29 ago 2026):** `INV_00` reescrita con el Director. Sale el marco de escasez (compadece, y compadecer baja el estatus de quien pregunta) y sale el párrafo que pedía disculpas. ⚠️ **Anclaje:** no se nombra *el paquete más grande* —mide por el precio y valida el costo como obstáculo—; se nombra **la entrada estándar / los tres paquetes principales**, y el Kit queda como *una puerta más liviana*. La diferencia del plan se da como hecho neutro.
...
### v6.29 — WHY_01 🔒 reescrito con el Director (9 sep 2026)

Auditoría a pedido del Director sobre la respuesta a «¿qué es CreaTuActivo?». Lo que salió: el marco del consumo diario (*«productos que las personas consumen todos los días»*, retirado de WHY_02 el 8 ago y sobreviviente aquí), el listado de la faena vieja (*explicarle a cada persona, responder a toda hora, estar pendiente de cada uno*), «negocio de distribución de café y suplementos» como sustantivo (el sistema se nombra por su categoría), y «se lo digo sin rodeos», que anuncia la propia franqueza. Lo que entró: la tesis del hero en el primer párrafo (dueño de su sistema de distribución de productos premium de bienestar con Ganoderma, desde el celular, cobra cada vez que mueve producto), el porqué a la manera de Ray Kroc —montar un negocio que requiere estar encima no es lo difícil, multiplicarlo sí, y ahí se quedaba casi todo el mundo— y los tres elementos ensamblados en un solo párrafo (fabricante · quien atiende · metodología de dos pasos, que no se nombran: es el hueco que entrega EAM_01). La tesis se dice una sola vez; en el «por qué existimos» el sujeto es el problema, y el sistema vuelve como «el suyo». ⚠️ El remate de ese párrafo iba a decir *«funciona sin que usted tenga que estar encima»* y la batería de negocio lo bloqueó (patrón de esfuerzo mínimo: *funciona … sin que usted*); en WhatsApp habría salido la correctiva en vez del candado. Quedó *«no depende de que usted esté encima»*, la forma aprobada de *no depende de su presencia*. Las tres baterías se corren ANTES de clonar, no después. Pregunta de seguimiento: «¿Le cuento cómo funciona el negocio?» → WHY_02 a 0.674, medido; la anterior («¿Le cuento cómo se gana?») caía en compensación. La cabecera dejó de decir que la categoría se bautiza después (criterio retirado el 25 ago) y recoge las reglas del estatus y de la faena en una frase. Desde el 9 sep el candado solitario lo emite el backend, así que este texto sale literal siempre.
