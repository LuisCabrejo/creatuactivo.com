...
async function aplicarArsenalManejo() {
  ...
  const contentMatch = fileContent.match(/content = '(.+)'\s*WHERE/s);
  if (!contentMatch) {
  ...
  console.log('📌 Título: Arsenal Manejo - Pendiente\n');
  ...
  const checks = [
    {
      name: 'OBJ_04: Frase "Conectar con mentor"',
      pattern: 'Conectar a las personas interesadas con tu mentor o alguien con experiencia',
      found: data[0].content.includes('Conectar a las personas interesadas con tu mentor o alguien con experiencia')
    },
    {
      name: 'OBJ_05: Frase "Conectar con alguien que explica mejor"',
      pattern: 'Conectar a las personas interesadas con alguien que les puede explicar mejor',
      found: data[0].content.includes('Conectar a las personas interesadas con alguien que les puede explicar mejor')
    },
    {
    ...
    {
      name: 'TECH_11: ACE (próximamente)',
      pattern: 'ACE (próximamente - asistente para preparar tus llamadas)',
      found: data[0].content.includes('ACE (próximamente - asistente para preparar tus llamadas)')
    },
    {
      name: 'TECH_15: "Director del sistema" (NO "Arquitecto")',
      pattern: 'Director del sistema',
      found: data[0].content.includes('Director del sistema') && !data[0].content.includes('Arquitecto:')
    }
  ];
