...
const emb=async t=>{const r=await fetch('https://api.voyageai.com/v1/embeddings',{method:'POST',headers:{'Authorization':`Bearer ${process.env.VOYAGE_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({input:[t],model:'voyage-3-lite',output_dimension:512,input_type:'query'})});return (await r.json()).data[0].embedding;};
