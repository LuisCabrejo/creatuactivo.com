 ...
 * en pequeño — ver PROYECCION_12 y nivel12Level). Respaldo del page anterior en
 * page.tsx.bak. ⚠️ Es una COPIA: cambios al deck base (/servilleta) deben replicarse
 * aquí a mano. El orbe Queswa se gatea con isDeck en UnifiedQueswaOrb.
 *
 * ↓ Historial heredado del deck servilleta v6.7 ↓
 *
...
export default function ServilletaPage() {
  ...
  const [nivel12Level, setNivel12Level] = useState(12); // Slide 4 · simulador de 12 niveles
  ...
  // Bolita del simulador de 12 niveles — crece de 20px (nivel 1) a 50px (nivel 12),
  // mismo rango que el snowball del INGRESO RECURRENTE.
  const nivel12ThumbSize = Math.round(20 + ((nivel12Level - 1) / 11) * 30);
    ...
    <>
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* --- Simulador 12 Niveles (Slide 4) --- */
        .nivel-track { display: flex; gap: 6px; justify-content: center; flex-wrap: wrap; margin: 4px 0 22px; }
        .nivel-dot {
          width: 34px; height: 34px; border-radius: 50%;
          background: transparent; border: 1px solid #444; color: #666;
          font-family: var(--font-mono); font-size: 0.8rem; font-weight: bold; cursor: pointer;
          display: flex; align-items: center; justify-content: center; transition: all 0.25s;
        }
        .nivel-dot:hover { border-color: #888; color: #aaa; }
        .nivel-dot.done { border-color: rgba(197,160,89,0.4); color: var(--orange); background: rgba(197,160,89,0.06); }
        .nivel-dot.active {
          border-color: var(--orange); color: #0F1115; background: var(--orange);
          box-shadow: 0 0 16px rgba(197,160,89,0.45); transform: scale(1.12);
        }
        .nivel-tag { text-align: center; font-family: var(--font-mono); font-size: 0.8rem; letter-spacing: 0.25em; color: var(--orange); margin-top: 4px; }
        .digital-display.cop-primary { font-size: clamp(1.7rem, 6vw, 2.6rem); color: var(--orange); margin: 6px 0 2px; }
        .digital-display.cop-primary .currency { color: var(--orange); font-size: 1.4rem; }
        .digital-display.cop-primary .unit { color: var(--orange); font-size: 1.1rem; }
        .nivel-users { text-align: center; color: #888; font-family: var(--font-mono); font-size: 0.9rem; margin-bottom: 18px; }
        ...
        @media (max-width: 768px) {
          ...
          #slide-3 .slide-3-layout { justify-content: flex-start !important; }
          ...
          #slide-3 .slide-3-bottom { padding-top: 10px !important; gap: 10px !important; }
          ...
          /* La nota del Dr. Leow sale SOLO en móvil: es el bloque más alto de la
             ...
             viva en escritorio, y en vivo el orador la narra igual. */
      ...
      <div className={`industrial-theme${isKiosk ? ' kiosk' : ''}`}>
        ...
        >
              ...
              <div className="slide-3-bottom">
                ...
                <div className="bio-metrics-container">
                  ...
                  {/* CTA debajo del panel de métricas. Oculto en móvil: allí la slide
                      se pasa deslizando y el botón se comía el alto que necesita la taza. */}
          ...
          <section id="slide-4" className={`slide ${activeSlide === 4 ? 'active' : ''}`}>
            <div className="simulator-layout">
              ...
              {/* Panel DERECHO — Simulador de 12 Niveles (reemplaza el botón ACTIVAR, jul 2026) */}
              <div className="simulator-panel">
                <h3 style={{ textAlign: 'center' }}>SIMULADOR &middot; LOS 12 NIVELES (2&times;2)</h3>
                ...
                <div className="nivel-track">
                  {PROYECCION_12.map((n) => (
                    <button
                      key={n.level}
                      type="button"
                      className={`nivel-dot ${n.level === nivel12Level ? 'active' : ''} ${n.level < nivel12Level ? 'done' : ''}`}
                      onClick={() => setNivel12Level(n.level)}
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div className="digital-display cop-primary">
                        <span className="currency">$</span>
                        <span>{sel.income.toLocaleString('es-CO')}</span>
                ...
                <div className="controls-container">
                  <label>
                    RECORRA LOS 12 NIVELES:
                    <span className="highlight-text">Nivel {nivel12Level}</span>
                  </label>
                  <input
                    type="range"
                    min={1}
                    max={12}
                    value={nivel12Level}
                    onChange={(e) => setNivel12Level(parseInt(e.target.value))}
                    className="sim-slider snowball-slider"
        ...
        {verticalMode && !isKiosk && (
          <div className="vertical-present-overlay" ref={vOverlayRef}>
            ...
            >
              <iframe src="/12-niveles?kiosk=1" title="Presentación vertical" />
            </div>
            <button
              type="button"
              className="vp-exit"
