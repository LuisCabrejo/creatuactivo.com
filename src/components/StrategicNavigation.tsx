...
export default function StrategicNavigation() {
  ...
  const handleDropdownMouseLeave = () => {
    const timeout = setTimeout(() => {
      setOpenDropdown(null)
    }, 100) // Delay más corto para el dropdown
    setHoverTimeout(timeout)
  }
  ...
  return (
    <header className="navigation-container sticky top-0 z-50 bg-slate-900/80 backdrop-blur-xl border-b border-white/10">
      ...
      {/* MENU MÓVIL */}
      {isMenuOpen && (
        <div className="mobile-menu-container md:hidden bg-slate-900/95 backdrop-blur-xl border-t border-white/10">
          <div className="px-4 pt-4 pb-6 h-full flex flex-col overflow-y-auto">
            {menuItems.map((item) => (
              <div key={item.name} className="py-2">
                {item.dropdown ? (
                  <>
                    ...
                    {item.dropdown.map((subItem) => (
                      <Link
                        ...
                        href={subItem.href}
                        onClick={handleLinkClick}
                        className="w-full flex items-center px-4 py-3 text-slate-300 hover:bg-slate-800/50 hover:text-white rounded-lg transition-colors"
                      >
                  ...
                  <Link
                    href={item.href}
                    onClick={handleLinkClick}
                    className="w-full flex items-center px-4 py-3 text-slate-300 hover:bg-slate-800/50 hover:text-white rounded-lg transition-colors font-medium"
                  >
                    {item.name}
                  </Link>
                )}
            ...
            <div className="mt-auto pt-4 border-t border-white/10">
              <Link
                href="/fundadores"
                onClick={handleLinkClick}
                className="w-full creatuactivo-cta-ecosystem text-center flex items-center justify-center gap-2"
              >
